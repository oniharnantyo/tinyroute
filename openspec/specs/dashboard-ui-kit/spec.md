...
All dashboard views (`view_*.templ`) SHALL build their UI from templui component packages (button, badge, card, input, table, selectbox, radio, breadcrumb, alert, skeleton, tabs, separator, dropdown, dialog, sheet) instead of hand-rolled markup with ad-hoc Tailwind class chains for those primitives.

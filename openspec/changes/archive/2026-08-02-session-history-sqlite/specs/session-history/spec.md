...
The system SHALL retrieve history records filtered by serving provider, by a
start timestamp, by an end timestamp, or by any combination thereof. Results
SHALL be returned in most-recent-first order and SHALL support cursor-based
pagination so that arbitrarily large result sets can be consumed in bounded
pages.

...
func TestOpencodeAdapter(t *testing.T) {
	...
	_, err := a.Apply(ApplyInput{
		...
		ModelSlots: map[string]string{
			"model":    "opencode-zen:big-pickle",
			"subagent": "opencode-zen:small-pickle",
		},
	})
	if err != nil {
	...
	if st.SlotValues["model"] != "opencode-zen:big-pickle" {
		t.Errorf("expected model slot opencode-zen:big-pickle, got %v", st.SlotValues["model"])
	}
	if st.SlotValues["subagent"] != "opencode-zen:small-pickle" {
		t.Errorf("expected subagent slot opencode-zen:small-pickle, got %v", st.SlotValues["subagent"])
	}

	cfgData, _ := os.ReadFile(st.ConfigPath)
	if !strings.Contains(string(cfgData), `"model": "tinyroute/opencode-zen:big-pickle"`) {
		t.Errorf("expected model tinyroute/opencode-zen:big-pickle in opencode.json, got:\n%s", string(cfgData))
	}
	if !strings.Contains(string(cfgData), `"model": "tinyroute/opencode-zen:small-pickle"`) {
		t.Errorf("expected explorer subagent tinyroute/opencode-zen:small-pickle in opencode.json, got:\n%s", string(cfgData))
	}

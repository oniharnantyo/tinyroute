				...
				for _, cm := range customModels {
					if modelObj, ok := cm.(map[string]any); ok {
						if id, ok := modelObj["id"].(string); ok && strings.HasPrefix(id, "custom:tinyroute") {
							pointed = true
							if bu, ok := modelObj["baseUrl"].(string); ok && bu != "" && currentBaseURL == "" {
								currentBaseURL = bu
							}
							if key, ok := modelObj["apiKey"].(string); ok && key != "" && maskedKey == "" {
								rawKey = key
								maskedKey = MaskKey(key)
							}
							if model, ok := modelObj["model"].(string); ok && model != "" {
								mNames = append(mNames, model)
								if slotValues["active"] == "" {
									slotValues["active"] = model
								}
							}
						}
					}
		...
		if existing, ok := m["customModels"].([]any); ok {
			for _, item := range existing {
				if modelObj, ok := item.(map[string]any); ok {
					if id, ok := modelObj["id"].(string); ok && strings.HasPrefix(id, "custom:tinyroute") {
						if k, ok := modelObj["apiKey"].(string); ok && k != "" {
							apiKey = k
							break
						}
					}

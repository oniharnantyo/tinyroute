...
func (h *hermesAdapter) Name() string     { return "Hermes Client" }
	...
	if installed {
		...
		if err == nil {
			if modelSec, ok := m["model"].(map[string]any); ok {
				...
				if defModel, ok := modelSec["default"].(string); ok && defModel != "" {
					slotValues["model"] = defModel
				}
			}
		}
		...
		if envData, err := os.ReadFile(envPath); err == nil {
			for _, line := range strings.Split(string(envData), "\n") {
				if strings.HasPrefix(line, "OPENAI_API_KEY=") {
					k := strings.TrimPrefix(line, "OPENAI_API_KEY=")
					rawKey = k
					maskedKey = MaskKey(k)
					break
				}
...
func (h *hermesAdapter) Apply(input ApplyInput) (Result, error) {
	...
	for i, l := range lines {
		if strings.HasPrefix(l, "OPENAI_API_KEY=") {
			lines[i] = fmt.Sprintf("OPENAI_API_KEY=%s", apiKey)
			found = true
			break
		}
	}

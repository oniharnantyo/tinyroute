...
func TestClaudeAdapter(t *testing.T) {
  ...
  "env": {
	...
	stAfter, err := adapter.Detect()
	if err != nil || !stAfter.PointedAtTinyRoute {
		t.Fatalf("expected pointed at tinyroute after Apply, st=%+v, err=%v", stAfter, err)
	}
	if stAfter.CurrentBaseURL != "http://127.0.0.1:8080/anthropic" {
		t.Errorf("expected CurrentBaseURL http://127.0.0.1:8080/anthropic, got %s", stAfter.CurrentBaseURL)
	}
	if !strings.Contains(stAfter.MaskedKey, "...") || strings.Contains(stAfter.MaskedKey, "testkey123") {
		t.Errorf("expected masked key without plaintext, got %s", stAfter.MaskedKey)
	}

	// 3. Reset (remove injected) re-apply
